...
export class BattleProcessorBattleStageApi {
    ...
    getWaveInfo() {
        ...
        const bossInfo = this.ctx.battleData.getBossInfo();
        return { bossInfo, wave, progress, maxWave, monsterCount, monsterLimit };
    }
    ...
    getForbidInfo() {
        const player = this.ctx.battleData.attackTeam.get(1);
        const frame = this.ctx.battleData.frame;
        if (frame < player.skillManager.blockInfo.endFrame) return player.skillManager.blockInfo.index;
        return [];
    }
