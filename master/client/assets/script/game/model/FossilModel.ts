...
export default class FossilModel {
    ...
    /**是否激活命星 */
    isStarActiveById(starId: number) {
        return BattleFactory.isStarActiveById(GState.data, starId);
    }
    /**获取命格属性描述 */
    ...
    /** 装备化石 */
    async equipStone(index: number, id: number) {
        if (id === -1) {
            await this.unEquipStone(index);
        }
        const formation = this.getFormation();
        const originIndex = formation.indexOf(id);
        if (originIndex > -1) {
            formation[originIndex] = -1;
            formation[index] = id;
        } else {
            formation[index] = id;
        }
        await this.setFormation(formation);
    }
    /** 卸下化石 */
    async unEquipStone(index: number) {
        const f = this.getFormation();
        f[index] = -1;
        await this.setFormation(f);
    }
    /**设置命格 */
