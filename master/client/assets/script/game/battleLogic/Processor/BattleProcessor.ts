...
export class BattleProcessor {
    ...
    /** 获取波数信息 */
    getWaveInfo() {
        const current = this.battleData.monsterQueue.getWave();
        const total = GTable.getById("MonsterWaveTbl", this.waveId).waveList.length;
        return {
            current,
            total,
        };
    ...
    /** 总击杀 */
    getTotalKill() {
        return this.battleData.globalPropertyManager.totalKill;
    }
