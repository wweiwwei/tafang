...
export class BattleRogueBlessManager {
    ...
    tick() {
        ...
        if (this.ctx.ctx.battleKind === "guide") {
            if ((wave >= 4 ? 1 : 0) > this.bless.length) {
                this.refreshCard();
            }
        } else {
            if (
                Math.floor((wave - 1) / GConstant.battle.blessWave) +
                    this.ctx.globalPropertyManager.baseProperty.initBless >
                this.bless.length
            ) {
                this.refreshCard();
            }
