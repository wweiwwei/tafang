...
export default class WindowCongratulation extends UIWindow {
    ...
    protected onInited(): void {
        ...
        let animation = content.getComponent(cc.Animation);
        ...
        if (this._windowParam.items !== undefined) {
            ...
            if (debris.length !== 0) {
                ...
                result.forEach(async (re) => {
                    let herotbl = GTable.getList("MountTbl").find((t) => t.frag === re.id);
                    state.unshift({ item: new Item(herotbl.frag, count), debris: true, status: 0 });
                    // let comp = await ResourceLoader.getNode(ListItemHeroItem);
                    // comp.node.scale = 0.7;
                    // comp.node.setContentSize(82, 82);
                    // this.heroitem.node.addChild(comp.node);
            ...
            if (count > 6) {
                ...
                this.heroitem.node.getComponent(cc.Layout).startAxis = cc.Layout.AxisDirection.HORIZONTAL;
            } else {
                if (count <= 5) this.heroitem.node.getComponent(cc.Layout).startAxis = cc.Layout.AxisDirection.VERTICAL;
