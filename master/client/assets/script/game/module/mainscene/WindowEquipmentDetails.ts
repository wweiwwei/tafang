...
export default class WindowEquipmentDetails extends UIWindow {
    ...
    @autowired(UILabel) txText: UILabel = null;
    ...
    @autowired(cc.Node) maxLevel: cc.Node = null;

    //改
    @autowired(cc.ProgressBar) lvProgress: cc.ProgressBar = null;
    @autowired(UILabel) proNumber: UILabel = null;
    ...
    refresh() {
        ...
        const sprite = GModel.sprite.getSpriteById(this._windowParam.id);

        if (sprite.uprankCost()) {
            // 还没满级
            //require:需要的
            const require = sprite.uprankCost();
            //storage:已经有的
            const storage = sprite.exp;
            this.lvProgress.progress = storage / require;
            this.proNumber.setText(["_rs" + storage.toString()], ["_rs/" + require.toString()]);
        } else {
            // 满级处理
        }
        ...
        this.txText.setText([mt.part == 0 ? "_rs武器特性" : "_rs装备特性"]);
        ...
        if (condition) {
            ...
            if (sprite.uprankCost()) {
                ...
                this.updateBtnCost.setState([{ item: item, require: item.count, storage: sprite.exp }]);
            } else {
        ...
        this.quality.setText(["_rs品质："], ["_rs" + GConstant.qualityName[it.quality]]);
        this.part.setText(["_rs部位："], ["_rs" + GConstant.partName[mt.part]]);
