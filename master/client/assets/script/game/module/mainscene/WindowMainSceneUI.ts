...
import WindowStageReward from "./WindowStageReward";
...
export default class WindowMainSceneUI extends UIWindow {
    ...
    @autowired(UIButton) lvRewardBtn: UIButton = null;
    @autowired(UIImage) lvRewardBtnTips: UIImage = null;
    ...
    refreshStage() {
        ...
        let rootNode: cc.Node = this.preStage.node.parent.parent;
        this.curStage.setText(info.current);

        let isMaxStage = () => {
            const afk = GModel.stage.getStageAfkMap();
            const stage = GState.data.stage[afk];
            let arr = GTable.getList("StageTbl").filter((t) => t.mapIndex == stage.mapIndex);
            if (stage.stageIndex == arr[arr.length - 1].stageIndex) return true;
            return false;
        };
        if (isMaxStage()) {
            rootNode.children[4].active = false;
            rootNode.children[3].active = false;
        } else {
            rootNode.children[4].active = true;
            rootNode.children[3].active = true;
        }
        if (info.pre) {
            rootNode.children[0].active = true;
            rootNode.children[1].active = true;
            this.raidBtn.node.active = true;
            this.preStage.node.active = true;
            this.preStage.setText(info.pre);
        } else {
            rootNode.children[0].active = false;
            rootNode.children[1].active = false;
            // 隐藏前置关卡
            this.raidBtn.node.active = false;
    ...
    // }
    even() {
        this.lvRewardBtn.onClick = () => {
            GWindow.open(WindowStageReward);
        };
    ...
    async refreshMission() {
        ...
        this.missionProgress.setText(["_rs<color=#000000>"], ...mission.getDescription(true), [
            "_rs</outline></color>",
        ]);
