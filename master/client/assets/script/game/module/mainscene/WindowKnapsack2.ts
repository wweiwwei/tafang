...
export default class WindowKnapsack2 extends UIWindow {
    ...
    @autowired(UILabel) itemsTiips: UILabel = null;
    @autowired(UILabel) itemsTiips2: UILabel = null;
    ...
    async refUi() {
        ...
        this.itemsTiips2.node.active = true;
        ...
        if (api) {
            ...
            if (this.menuIndex == 0) {
                ...
                if (data.length > 0) {
                    ...
                    // console.log("touid =", this.touchId);
                    let obj = data.find((d) => d.id == this.touchId);
                    let tbl = GTable.getById("RogueEquipmentTbl", obj.uid);
                    this.switchGoods.setState([
                        {
                            id: this.touchId,
                            cb: cb,
                            image: tbl.img,
                            bg: GConstant.itemQualityBg[tbl.quality],
                            imgScale: 1,
                        },
                    ]);
                    this.itemsTiips2.node.active = false;
                ...
                items = data.map((d) => {
                    ...
                    return { id: d.id, cb: cb, image: image, bg: bg, selected1: d.id == this.touchId };
                });
            }
            // 获取战斗祝福列表
                ...
                if (data.length > 0) {
                    ...
                    this.switchGoods.setState([
                        { id: this.touchId, cb: cb, image: tbl.img, bg: "battle_bless_bg", imgScale: 1.2 },
                    ]);
                    this.goodsName.setText([tbl.name]);
                    this.goodsDesc.setText([tbl.description]);
                    this.itemsTiips2.node.active = false;
                ...
                items = data.map((d) => {
                    ...
                    return { id: d, cb: cb, image: image, bg: bg, selected2: d == this.touchId };
                });
            }
        }
        if (items.length > 0) {
        ...
        this.itemsTiips.node.active = items.length == 0;
    }
