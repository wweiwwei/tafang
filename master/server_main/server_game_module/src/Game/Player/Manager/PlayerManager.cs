...
namespace GamePlay
{
    public class PlayerManager : AbstractManager<PlayerData>, IBaseManager
    {
        ...
        {
            if (Ctx.Now() - Data.lastPowerRefresh > Ctx.Config.Player.PowerRecoverInterval)
            {
                var canRecover = (Ctx.Now() - Data.lastPowerRefresh) / Ctx.Config.Player.PowerRecoverInterval;
                if (Data.power > Ctx.Config.Player.PowerMax)
                {
                    Data = Data with { lastPowerRefresh = Data.lastPowerRefresh + canRecover * Ctx.Config.Player.PowerRecoverInterval };
                }
                else
                {
                    var finalPower = Math.Min(Data.power + canRecover, Ctx.Config.Player.PowerMax);
                    Data = Data with { power = (int)finalPower, lastPowerRefresh = Data.lastPowerRefresh + canRecover * Ctx.Config.Player.PowerRecoverInterval };
                }
                Ctx.Emit(CachePath.powerData);
            }
        }

        public void UsePower(int count)
        {
            var cost = Ctx.Config.Player.PowerCost * count;
            GameAssert.Expect(cost < Data.power, 20011);
            Data = Data with { power = Data.power - cost };
            Ctx.Emit(CachePath.powerData);
        }

        [Handle("power/videoAddPower")]
        public void VideoAddPower()
        {
            GameAssert.Expect(Data.todayVideoPower < Ctx.Config.Player.PowerVideo[1], 20012);
            Data = Data with
            {
                power = Data.power + Ctx.Config.Player.PowerVideo[0],
                todayVideoPower = Data.todayVideoPower + 1
            };
            Ctx.Emit(CachePath.powerData);
        }
        [Handle("power/diamondAddPower")]
        public void DiamondAddPower(int count)
        {
            GameAssert.Expect(Data.todayDiamondPower + count <= Ctx.Config.Player.PowerDiamond[2], 20013);
            Ctx.KnapsackManager.SubItem(new Item(GameConstant.DiamondId, count * Ctx.Config.Player.PowerDiamond[1]));
            Data = Data with
            {
                power = Data.power + Ctx.Config.Player.PowerDiamond[0],
                todayDiamondPower = Data.todayDiamondPower + count
            };
            Ctx.Emit(CachePath.powerData);
        }

        [Update("powerData")]
        public Dictionary<string, object> PowerData()
        {
            return new()
            {
                {"power",Data.power},
                {"lastPowerRefresh",Data.lastPowerRefresh},
                {"todayVideoPower",Data.todayVideoPower},
                {"todayDiamondPower",Data.todayDiamondPower},
            };
        }
