...
import { BattleRogueBossManager } from "./Rogue/BattleRogueBossManager";
...
export class BattleBattleStageData {
    ...
    constructor(public ctx: BattleProcessor) {
        ...
        this.rogueBossManager = new BattleRogueBossManager(this);
        ...
        this.mapInfo = new BattleMapInfo(ctx.mapId);
    ...
    rogueBossManager: BattleRogueBossManager;
    ...
    addMonster(m: BattleBattleStageObject) {
        ...
        if (m.propertyManager.heroTag.includes("boss") || m.propertyManager.heroTag.includes("elite")) {
            this.pushDisplayEvent(new BattleDisplayEvent("showBossComming", {}));
            if (m.propertyManager.heroTag.includes("boss")) {
                this.boss = m;
                this.bossEndTick = this.frame + GConstant.battle.bossTime * GConstant.battle.logicTickRate;
            }
        }
    }

    boss: BattleBattleStageObject;
    bossEndTick: number = 999999999999;
    ...
    getBossInfo(): {
        ...
        if (this.boss) {
            return {
                bossOnScene: true,
                hp: this.boss.propertyManager.hp,
                maxHp: this.boss.propertyManager.finalProperty.maxHp,
                tickRemain: this.bossEndTick - this.frame,
            };
        } else {
            return {
                bossOnScene: false,
    ...
    objectDie(obj: BattleBattleStageObject, withAnimation = true) {
        ...
        if (obj.teamSide === 0) {
            ...
            this.globalPropertyManager.monsterDie(obj);
