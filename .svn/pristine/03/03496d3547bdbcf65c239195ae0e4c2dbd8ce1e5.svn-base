...
import { BattleBossEffectInfo, BattleDisplayToLogicApi } from "../../../battleApi/BattleDisplayToLogicApi";
...
import WindowBossComming from "../rogue/WindowBossComming";
...
export default class BattleSceneDisplay extends cc.Component implements BattleDisplayToLogicApi {
    ...
    showBossEffect(data: BattleBossEffectInfo): void {
        EventBus.emit(EventName.showBossEffect, data);
    }

    async showBossComming(data: {}) {
        GModel.battle.pauseBattle(true);
        await GWindow.open(WindowBossComming);
        GModel.battle.pauseBattle(false);
    }

    showArtifactEffect(data: { id: number }) {
        GTip.showArtifactEffect(data.id);
    }

    bossSkillDialogue(data: { id: number }) {
        EventBus.emit(EventName.bossSkillDialogue, data.id);
    }
}
