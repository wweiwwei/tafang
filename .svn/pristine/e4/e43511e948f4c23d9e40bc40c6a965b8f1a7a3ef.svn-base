...
function requireHttpErrors() {
    if (hasRequiredHttpErrors) return httpErrorsExports;
...
function requireSrc$1() {
    if (hasRequiredSrc$1) return srcExports;
    hasRequiredSrc$1 = 1;
    (function (module) {
...
var gbChars = [
    ...
    39113, 39114, 39115, 39116, 39265, 39394, 189000,
];
var require$$4 = {
    uChars: uChars,
    gbChars: gbChars,
...
function requireLib$1() {
    if (hasRequiredLib$1) return libExports$1;
    hasRequiredLib$1 = 1;
    (function (module) {
...
function requireTypeIs() {
    if (hasRequiredTypeIs) return typeIsExports;
    hasRequiredTypeIs = 1;
    ...
    typeIsExports.is = typeis;
    typeIsExports.hasBody = hasbody;
    typeIsExports.normalize = normalize;
...
})(bodyParser$1, bodyParserExports);
...
var debug$5 = requireSrc$1()("finalhandler");
...
var debug$4 = requireSrc$1()("express:router:layer");
...
function getCurrentNodeMethods() {
    return (
        http$2.METHODS &&
        http$2.METHODS.map(function lowerCaseMethod(method) {
...
var debug$3 = requireSrc$1()("express:router:route");
...
var debug$2 = requireSrc$1()("express:router");
...
var debug$1 = requireSrc$1()("express:view");
...
var contentDisposition$2 = {
    get exports() {
        return contentDispositionExports;
    },
    set exports(v) {
        contentDispositionExports = v;
    },
};
...
SendStream.prototype.redirect = function redirect(path) {
    ...
    var doc = createHtmlDocument(
        "Redirecting",
        'Redirecting to <a href="' + escapeHtml$1(loc) + '">' + escapeHtml$1(loc) + "</a>"
    );
...
function createHttpError(status, err) {
    ...
    return err instanceof Error ? createError$1(status, err, { expose: false }) : createError$1(status, err);
}
    ...
    if (keys.length > 0) {
        if (
            keys.every(function (k) {
                return (
                    spec.params[k] == "*" || (spec.params[k] || "").toLowerCase() == (p.params[k] || "").toLowerCase()
                );
            })
        ) {
            s |= 1;
...
defineGetter(
    req,
    "host",
    deprecate$1.function(function host() {
...
app.get("/getBattleSkillFile/:fileName", (req, res) => {
    const fileName = req.params.fileName;
    const data = require$$1$2.readFileSync("../table/battleSkill/" + fileName + ".json", "utf-8");
    res.send(data);
});
app.post("/writeBattleSkillFile/:fileName", (req, res) => {
    const fileName = req.params.fileName;
    const data = req.body;
    require$$1$2.writeFileSync("../table/battleSkill/" + fileName + ".json", JSON.stringify(data), "utf-8");
    res.send({});
});
app.listen(16691, () => {
    console.log("表格服务器成功启动,端口号16691");
});
