...
export default class ListItemStage extends UIListItem {
    ...
    @autowired(UIButton) sweep: UIButton = null;
    ...
    refresh() {
        ...
        this.enter.onClick = () => {
            ...
            GModel.stage.challengeStage(this.state.mapIndex, this.selected);
        };
        this.sweep.node.active = stage.clear || stage.stageIndex > this.selected;
        this.sweep.onClick = () => {
            if (stage.stageIndex < this.selected) {
                GTip.showTip(["_rs请先挑战所有前置关卡"]);
                return;
            }
            GModel.stage.sweep(this.state.mapIndex, this.selected);
        };
        ...
        this.selectList.setState(
            stageTblList.map((t) => {
                return {
                    ...
                    selected: t.stageIndex === this.selected,
                    cb: () => {
                        if (stage.stageIndex < t.stageIndex) {
                            GTip.showTip(["_rs请先挑战所有前置关卡"]);
                            return;
                        }
                        this.selected = t.stageIndex;
                        this.refresh();
                    },
                };
            })
