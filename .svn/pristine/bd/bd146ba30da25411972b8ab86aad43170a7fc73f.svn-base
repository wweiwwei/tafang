...
export default class WindowBattle extends UIWindow {
    ...
    refreshWaveInfo(api: BattleProcessorBattleStageApi, dt: number) {
        ...
        if (!this.currentDialogue.hasShow) {
            ...
            cc.Tween.stopAllByTarget(this.bossWord.node.parent);
            this.bossWord.node.parent.x = 600;
            cc.tween(this.bossWord.node.parent).to(0.5, { x: -10 }, { easing: "sineInOut" }).start();
            const dialogueTbl = GTable.getById("BossDialogueConfigTbl", bossTbl.dialogue);
