...
namespace GamePlay
{
    public class CacheHandler
    {
        ...
        private ImmutableHashSet<System.Int32> exSkillLv_keys = ImmutableHashSet<System.Int32>.Empty;
        ...
        private ImmutableHashSet<System.Int32> skillLv_keys = ImmutableHashSet<System.Int32>.Empty;
        ...
        public INotifyMessage CacheHandle(string path, bool partial, ImmutableHashSet<dynamic> keySet)
        {
            switch (path)
            {
                ...
                case "curExSkill":
                    {
                        var data = Ctx.RogueManager.CurExSkill();
                        return new NotifyCacheMessage(path, data);
                    }
                ...
                case "exSkillLv":
                    {
                        var data = Ctx.RogueManager.ExSkillLv();
                        if(partial)
                        {
                            var patch = data.Where(kv => keySet.Contains(kv.Key)).ToImmutableDictionary(kv => kv.Key, kv => kv.Value);
                            var curKeySet = data.Keys.ToImmutableHashSet();
                            var delKey = exSkillLv_keys.Except(curKeySet).ToImmutableHashSet();
                            exSkillLv_keys = curKeySet;
                            return new NotifyPartialCacheMessage(path, patch, delKey, partial);
                        }
                        else
                        {
                            exSkillLv_keys = data.Keys.ToImmutableHashSet();
                            return new NotifyPartialCacheMessage(path, data, ImmutableHashSet<object>.Empty, partial);
                        }
                    }
                case "formation":
                ...
                case "mountCurrent":
                    {
                        var data = Ctx.PlayerMountManager.MountCurrent();
                        return new NotifyCacheMessage(path, data);
                    }
                case "mountStorage":
                    {
                        var data = Ctx.PlayerMountManager.MountStorage();
                        return new NotifyCacheMessage(path, data);
                    }
                case "petFormation":
                    {
                ...
                case "powerData":
                    {
                        var data = Ctx.PlayerManager.PowerData();
                        return new NotifyCacheMessage(path, data);
                    }
                case "rank":
                    {
                ...
                case "skillLv":
                    {
                        var data = Ctx.RogueManager.SkillLv();
                        if(partial)
                        {
                            var patch = data.Where(kv => keySet.Contains(kv.Key)).ToImmutableDictionary(kv => kv.Key, kv => kv.Value);
                            var curKeySet = data.Keys.ToImmutableHashSet();
                            var delKey = skillLv_keys.Except(curKeySet).ToImmutableHashSet();
                            skillLv_keys = curKeySet;
                            return new NotifyPartialCacheMessage(path, patch, delKey, partial);
                        }
                        else
                        {
                            skillLv_keys = data.Keys.ToImmutableHashSet();
                            return new NotifyPartialCacheMessage(path, data, ImmutableHashSet<object>.Empty, partial);
                        }
                    }
                case "skinCurrent":
                    {
                        var data = Ctx.PlayerSkinManager.SkinCurrent();
                        return new NotifyCacheMessage(path, data);
                    }
                case "skinStorage":
                    {
                        var data = Ctx.PlayerSkinManager.SkinStorage();
                        return new NotifyCacheMessage(path, data);
                    }
                ...
                case "stageAdInfo":
                    {
                        var data = Ctx.StageManager.StageAdInfo();
                        return new NotifyCacheMessage(path, data);
                    }
                case "stageAfkMap":
                    {
                ...
                case "stageFirstReward":
                    {
                        var data = Ctx.StageManager.StageFirstReward();
                        return new NotifyCacheMessage(path, data);
                    }
                case "stageWaveRecord":
                    {
                        var data = Ctx.StageManager.StageWaveRecord();
                        return new NotifyCacheMessage(path, data);
                    }
                case "stone":
                    {
