...
import { BattleSkillTemplateEnergyRecoverBuilder } from "../Builder/Template/BattleSkillTemplateEnergyRecoverBuilder";
import { BattleEnergyRecover } from "../../Entity/BattleEnergyRecover";
import { BattleTriggerEvent } from "../../Event/BattleTriggerEvent";

export class BattleSkillTemplateEnergyRecover extends BattleSkillBaseTemplate {
    process(ctx: BattleBattleStageContext, e: BattleTriggerEvent): void {
        const targetList = this.getTarget(ctx, e);
        const b = this.builder as BattleSkillTemplateEnergyRecoverBuilder;
        ...
        if (parallel) {
            const res = targetList
                ...
                .map((o, i) => {
                    ...
                    const r = BattleEnergyRecover.createEnergyRecover(ctx.object, o, baseValue, p);
                    let effect = this.buildEffect(ctx.object, o, i, parallel);
                    let display = effect.concat(o.propertyManager.handleEnergyRecover(r));
                    return { display, o, r };
                });
