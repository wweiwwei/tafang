...
export default class WindowVictoryAndFailure extends UIWindow {
    ...
    initUi() {
        ...
        this.chapter.setText([`_rs`], [mapTbl.name]);
        this.lv.setText([`_rs${this._windowParam.mapIndex}-${this._windowParam.stageIndex}`]);
        const info = this._windowParam.processor.getWaveInfo();
        this.waveOrder.setText([`_rs${info.current}/${info.total}`]);
        this.killMonsters.setText([`_rs${this._windowParam.processor.getTotalKill()}`]);
        this.adBtn.text.setText([`_rs(${GModel.stage.remainVideoDouble()}/${GConfig.stage.doubleDaily})`]);

        let state = this._windowParam.items.map((d) => {
            return { item: d, status: 0 };
        });
        this.rewardList.setState(state);
        this.adBtn.node.active = !this.hasViewAd;
        this.adBtn.setGrey(GModel.stage.remainVideoDouble() <= 0);
    }

    private hasViewAd = false;

    //副本打开返回副本，主页打开返回主页
    even() {
        this.adBtn.onClick = async () => {
            await GSDK.showVideo("double_stage_reward");
            const { reward } = await GModel.stage.challengeVideoReward();
            this._windowParam.items = reward;
            this.hasViewAd = true;
            this.initUi();
        };
