...
export default class BattleModel {
    ...
    toggleBattleSpeed() {
        if (GSDK.isTest) {
            if (this.battleSpeed === 1) {
                ...
                this.battleSpeed = 4;
            } else if (this.battleSpeed === 4) {
                this.battleSpeed = 8;
            } else if (this.battleSpeed === 8) {
                this.battleSpeed = 16;
            } else if (this.battleSpeed === 16) {
                this.battleSpeed = 1;
            }
        } else {
            if (this.battleSpeed === 1) {
    ...
    init() {
        [
            ...
            EventName.stateKey.skillLv,
            EventName.stateKey.exSkillLv,
            EventName.stateKey.mountStorage,
            EventName.stateKey.mountCurrent,
            EventName.stateKey.skinStorage,
            EventName.stateKey.skinCurrent,
        ].forEach((e) => {
    ...
    /** 引导战斗 */
    createGuideBattle(): BattleProcessor {
        const infos = this.playerTeamInfo();
        const builder = new BattleProcessorBuilder()
            .setAttackTeam(infos)
            .setCommandGetter(GBattleApiManager.getCommandGetter())
